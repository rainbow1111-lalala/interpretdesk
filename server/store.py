...
def start_meeting(title: str = "") -> int:
...
def add_turn(meeting_id: int, turn_id: int, src: str, dst: str, src_lang: str) -> None:
    with _conn() as conn:
        conn.execute(
...
def get_turns(meeting_id: int) -> list[dict]:
    with _conn() as conn:
        rows = conn.execute(
