...
from server import config, context_store, retrieval
...
async def main() -> int:
    ...
    context_store.save(WS, s.context)
    retrieval.store_doc(WS, "上一场底稿", "上一场会的原文内容")
    WS.index.write_text('{"model":"x","entries":[{"doc":"上一场底稿"}]}', encoding="utf-8")
    s.excerpts = [{"doc": "上一场底稿", "text": "上一场会的片段"}]

    builder = TurnBuilder(emit, s.context.glossary(), cumulative=True)
    s.builder = builder
    ...
    # 点「清空全部底稿」
    info = await srv.clear_context(s)

    checks = {
        "磁盘原文已清空": info["docs"] == [],
        "原文索引已删除": not WS.index.exists(),
        "底稿摘要已清空": info["matter"] == "" and info["glossarySize"] == 0,
        "内存预取片段已清空": s.excerpts == [],
        "原文挪进了回收站": len(list(WS.trash.rglob("*.txt"))) == 1,
    }
